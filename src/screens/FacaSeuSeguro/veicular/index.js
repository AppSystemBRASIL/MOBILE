...
const FazerSeguro = ({ navigation, route }) => {
  const { corretor, corretora, seguradoras, getSeguradoras } = useContext(Context);
  ...
  const Body = () => {
    ...
    const enviarSeguro = async () => {
      ...
      if(tipo !== 'veicular') {
        if((!nomeCompleto) || (!validatePhone(celular))) {
          if(!nomeCompleto) {
            addError('nomeCompleto');
          }
      ...
      if(tipo !== 'veicular') {
        ...
        dataSend = {
          ...
          seguro: {
            tipo: tipoSeguro,
            seguradora: [...seguradoras].filter(e => e.uid === seguradora)[0].razao_social,
        ...
        {page === 0 ? (
          <>
            <FormControl style={{marginTop: 20}} isInvalid={errors.find(response => response === 'nomeCompleto')}>
              <Text style={{ fontSize: 20, fontWeight: '600', marginBottom: 5 }}>SEGURADO:</Text>
              ...
              }} autoCapitalize='characters' autoCorrect={false} autoCompleteType='off' placeholder='Seu nome' onChangeText={(e) => setNomeCompleto(maskLetters(String(e).toUpperCase().normalize('NFD').replace(/[\u0300-\u036f]/g, "")))} onSubmitEditing={() => {
                if(nomeCompleto) {
                ...
                <FormControl style={{marginTop: 20}} isInvalid={errors.find(response => response === 'seguradora')}>
                  ...
                  >
                    {[...seguradoras]?.map((item, index) => (
                      <Select.Item label={item.razao_social} value={item.uid} key={index} />
                    ))}
                  </Select>
                  <FormControl.ErrorMessage leftIcon={<WarningOutlineIcon size="xs" />}>
                    Campo inválido
